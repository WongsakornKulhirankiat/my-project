...
class Screen2(tk.Frame):
    def __init__(self, parent, main_app):
        ...
        # Add "<" Button
        back_btn = customtkinter.CTkButton(master=self.password_frame, command= lambda: self.next_screen(1), width=30, height=30, text="<", text_color="white", font=("Arial", 30), fg_color="transparent", hover=False)
        ...
        # Add Hidden Button
        next_btn = customtkinter.CTkButton(master=self.password_frame, command= lambda: self.next_screen(3), fg_color="transparent", text="", hover=False, cursor="hand2", width=300, height=40)
        next_btn.place(x=30, y=200)

        # Add Description Label
        description_label = customtkinter.CTkLabel(self.password_frame, text_color="#a8a8a8", text="Enter current or initial password", font=("Arial", 23))
        description_label.place(x=23, y=270)
        ...
        def add_to_password(number):
            ...
            self.intial_password += str(number)
        ...
        def delete_from_password():
            if self.intial_password:
                # Delete Intial Password Right Position
                self.intial_password = self.intial_password[:-1]

                # Delete Intial Password From Screen
    ...
    def clear_password(self, frame):
        ...
        for child in frame.winfo_children():
            if isinstance(child, customtkinter.CTkLabel) and child.cget("text") not in ["Password", "Enter current or initial password"]:
                child.destroy()

    def display_password(self):
        
        # Show Password In Terminal
        print("Intial Password:", self.intial_password)

        # Check length password
        if (self.count < 6):

            if len(self.intial_password) >= self.count + 1:

                # Add Password on Screen
                intial_password_label = customtkinter.CTkLabel(self.password_frame, text_color="#a8a8a8", text=self.intial_password[self.count], font=("Arial", 30))
                intial_password_label.place(x=43 + self.count * 50, y=205)
    ...
    def check_password(self):
        ...
        if self.intial_password == self.current_password:
