...
def get_usb_drive():
    drives = [disk.mountpoint for disk in psutil.disk_partitions() if 'media' in disk.mountpoint]
    print(drives)  # Added print statement to display the drives
    if drives:
        return drives[0]  # Return the first found removable drive mount point
    return None
...
def display_images(image_paths):
    ...
    if image_paths:
        ...
        image_label.config(text="No Pictures Found")
