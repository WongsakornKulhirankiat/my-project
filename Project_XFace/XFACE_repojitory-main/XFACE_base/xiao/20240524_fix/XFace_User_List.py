...
from io import BytesIO  ###xiao_fix 20240523 + BytesIO 
...
class Screen13(tk.Frame):
    def __init__(self, parent, main_app):
        ...
        # Portraits List
        self.portraits_image = []
        # Toggle Confirm
        self.confirm = 0
        
        ###xiao_fix 20240523 delete
        ...
        # Add "<" Button
        back_btn = customtkinter.CTkButton(master=self.name_frame, command= lambda: self.back_screen(), width=30, height=30, text="<", text_color="black", font=("Arial", 30), fg_color="transparent", hover=False)
        ...
        self.username_entrybox = customtkinter.CTkEntry(master=self.name_frame, width=200 , height=30, fg_color="white", border_color="white", text_color="black", font=customtkinter.CTkFont(size=15), corner_radius=10)
    ...
    def search(self):
        ...
        username = self.username_entrybox.get()   ###xiao_fix 20240524 user search
        if username == '':
    ...
    def toggle_detail_frame(self, toggle, *args, **kwargs):
        if toggle == 1:
            ...
            name = kwargs.get('b', "")
            photo_data = XFace_Database_Function.fetch_facephoto_by_username(name)   ###xiao_fix 20240523 データベースから顔写真を読み取り
            ...
            canvas_detail = customtkinter.CTkCanvas(detail_frame, width=175, height=175, bg="white", highlightthickness=0, bd=0)
            canvas_detail.create_aa_circle(88, 89, 86, math.pi/2, fill=border_color)
            canvas_detail.create_image(87, 87, image=self.portrait_image, anchor=tk.CENTER)
            ...
            edit_btn = customtkinter.CTkButton(master=detail_frame, command=lambda: self.next_screen(name, 14), text="Edit", text_color="black", hover_color="lightgray", font=("Arial", 20), border_width=0, width=90, height=30, fg_color="#d9d9d9")
    ...
    def ok_frame(self, name):
        self.confirm = 0
        XFace_Database_Function.delete_user_by_username(name)
        #self.main_app.deletepickle_save(userid)   ###fujiwara syuusei(yet)  useridwo syutokusitai
        self.toggle_detail_frame(0)
        users= XFace_Database_Function.fetch_usernames() 
        self.build_user_frames(users)
        self.confirm_frame.destroy()
    ...
    def next_screen(self, name, index):
        list = XFace_Database_Function.fetch_userinfo_departmentname_by_username(name)
