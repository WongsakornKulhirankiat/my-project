...
def fetch_facephoto_by_username(user_name):          ###xiao_fix 20240523 facephoto get
    conn = sqlite3.connect(userRegPhoto_directory)
    cursor = conn.cursor()
...
def fetch_username_by_username(user_name):  ###xiao_fix 20240524 username search
    conn = sqlite3.connect(userRegPhoto_directory)
    cursor = conn.cursor()
    cursor.execute("SELECT UserName FROM userinfo WHERE UserName=?",(user_name,))
...
def delete_user_by_username(username):
    conn = sqlite3.connect(userRegPhoto_directory)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM userinfo WHERE UserName = ?", (username,))   ###fujiwara syuusei(yet)  kankeinai usermadesakujosareru?
    conn.commit()
...
def fetch_usernames_by_departmentname_username(department_name,user_name):   ###xiao_fix 20240524 search
    conn = sqlite3.connect(userRegPhoto_directory)
    cursor = conn.cursor()
    query = """
    SELECT userinfo.UserName
