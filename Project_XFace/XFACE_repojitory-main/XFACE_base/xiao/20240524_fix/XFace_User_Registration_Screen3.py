...
class Screen9(tk.Frame):
    def __init__(self, parent, main_app):
        ...
        # Add "<" Button                                                                            ###xiao_fix 20240523  15 delete
        back_btn = customtkinter.CTkButton(master=self.step_frame, width=30, height=30, command= lambda: self.next_screen(), text="<", text_color="black", font=("Arial", 30), fg_color="transparent", hover=False)
        ...
        profile_path = os.path.join(current_directory, userRegPhoto_path)   ###xiao_fix 20240523 撮った写真のパスを取得
        ...
        profile_image = profile_image.resize((150, 150), Image.Resampling.LANCZOS) ###xiao_fix 20240523 画像精度を確保の関数
        ...
        self.profile_label.image = profile_photo  # Keep a reference to avoid garbage collection
        self.profile_label.place(x=410, y=110)
        ...
        # Add Administrator Label                                                                                       ###xiao_fix 20240523 size 30->15
        self.administrator_check_label = customtkinter.CTkLabel(master=self.step_frame2, width=30, height=30, text="", font=customtkinter.CTkFont(size=15), text_color="black", justify="center")
        self.administrator_check_label.place(x=275, y=255)

        # Add Button "Save"
        btn_save = customtkinter.CTkButton(master=self.step_frame2, command=lambda: self.save(), width=80, text="Save", font=customtkinter.CTkFont(size=20), text_color="black", corner_radius=15, fg_color="white", border_color="white", border_width=1)
    ...
    def save(self):
        ...
        if admin_text == 'True': ###xiao_fix 20240523 O -> True  X -> False
            is_admin = 1
        elif admin_text == 'False': 
            is_admin = 0
        face_photo = self.main_app.get_registerfacephoto()
        XFace_Database_Function.insert_userinfo(username, password, department, is_admin, face_photo)
        self.main_app.pickle_save(userid,username,is_admin)
        messagebox.showinfo("Update", "Updated successfully!")

        loginuseradminflag = self.main_app.get_loginuseradminflag()  ###xiao_fix 20240523 管理者判定の追加
    ...
    def next_screen(self):
        self.main_app.show_next_screen(7) ###xiao_fix 20240523 index -> 7 

    """def get_userid(self):
        self.userid_txt = self.main_app.get_userid()
    ...
    def get_administrator(self):
        self.administrator_check_txt = self.main_app.get_administrator()    ###xiao_fix 20240523 追加O -> True  X -> False
